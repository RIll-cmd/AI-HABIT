...
import { Volume2, VolumeX, Headphones, Radio, Sparkles } from "lucide-react";
...
const SOUND_OPTIONS: { id: AmbientSoundType; label: string; icon: string; desc: string }[] = [
  ...
  { id: "RAIN", label: "Cyber Rain", icon: "🌧️", desc: "Neon rainfall on glass" },
...
export const AmbientSoundPlayer: React.FC<{ className?: string }> = ({ className = "" }) => {
  ...
  // Initialize Web Audio Synthesis for ambient soundscapes
  useEffect(() => {
    ...
    try {
      ...
      if (ambientSound === "RAIN" || ambientSound === "LOFI_NOISE") {
        ...
        filter.frequency.setValueAtTime(ambientSound === "RAIN" ? 1000 : 400, ctx.currentTime);
        filter.Q.setValueAtTime(ambientSound === "RAIN" ? 1.5 : 0.8, ctx.currentTime);
