...
import { BestiarySpeciesSummary, BeastRarity, BeastElement } from "@/features/beasts/types/beast";
...
export const BeastBestiary: React.FC<BeastBestiaryProps> = ({
  bestiary,
  characterId,
  totalDiscovered,
  totalSpecies,
  ...
  const [selectedBuff, setSelectedBuff] = useState<string>("ALL");
  ...
  const buffCategories = [
    { id: "ALL", label: "All Buffs" },
    { id: "EXP_BOOST", label: "EXP Boost" },
    { id: "GOLD_BOOST", label: "Gold Boost" },
    { id: "STR_BOOST", label: "Strength Boost" },
    { id: "AGI_BOOST", label: "Agility Boost" },
    { id: "INT_BOOST", label: "Knowledge Boost" },
  ];
  ...
  const filteredBeasts = useMemo(() => {
    return bestiary.filter((b) => {
      ...
      if (selectedBuff !== "ALL") {
        const type = b.statBonusType;
        if (selectedBuff === "EXP_BOOST" && !type.includes("EXP")) return false;
        if (selectedBuff === "GOLD_BOOST" && !type.includes("GOLD")) return false;
        if (selectedBuff === "STR_BOOST" && !type.includes("STR") && !type.includes("DISCIPLINE")) return false;
        if (selectedBuff === "AGI_BOOST" && !type.includes("AGI") && !type.includes("FOCUS")) return false;
        if (selectedBuff === "INT_BOOST" && !type.includes("INT") && !type.includes("KNOWLEDGE")) return false;
      }
      return true;
    });
  }, [bestiary, selectedRarity, selectedBuff]);

  const handleEquipClick = async (b: BestiarySpeciesSummary) => {
    if (!b.isUnlocked || !b.beastInstanceId) return;
    if (b.isEquipped) {
      await equipBeast(characterId, null);
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-3xl bg-gradient-to-br from-[#0C1226]/95 via-[#080E20]/95 to-[#050914]/98 border border-cyan-500/30 p-6 shadow-xl relative overflow-hidden backdrop-blur-2xl">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-mono font-bold text-cyan-400 uppercase tracking-widest flex items-center gap-1.5">
                ...
                STEPS & BEASTS BESTIARY CODEX
              </span>
            </div>
            <h2 className="text-xl sm:text-2xl font-black font-heading text-white tracking-wide">
              Companion Creature Bestiary
            </h2>
            <p className="text-xs text-slate-400 font-mono">
              Hatch eggs with real physical steps to unlock dragons and activate passive resonance buffs.
            </p>
          </div>
        ...
        {/* Filter Tabs */}
        <div className="relative z-10 flex flex-col gap-3 pt-5 border-t border-cyan-500/10 mt-5">
          {/* Rarity Filters */}
          ...
          {/* Passive Buff Filters */}
          <div className="flex flex-wrap items-center gap-2">
            {buffCategories.map((b) => (
              <button
                key={b.id}
                onClick={() => {
                  playUIMenuSFX("confirm");
                  setSelectedBuff(b.id);
                }}
